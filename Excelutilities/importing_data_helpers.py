...
def add_col_data_from_another_sheet(col_name_1, col_name_2, allowed_data_types=[float, int, decimal.Decimal,datetime.datetime, str]):
    ...
    sg.popup_ok(f"Please select the input {col_name_1} and click 'ok' when finished", keep_on_top=True)
    ...
    sg.popup_ok(f"Please select the input {col_name_2} and click 'ok' when finished", keep_on_top=True)
    ...
    def sanity_check_col_entries(entry1, entry2, addresses_and_names):
        ...
        for address_and_name in addresses_and_names:
            ...
            if not is_col_block_bool(address):
                print(address)
                sg.popup(f"Oops! Your {name} data wasn't from a single column", keep_on_top=True)
                sys.exit()
    ...
    sg.popup_ok(f"Please select the {col_name_1} and click 'ok' when finished", keep_on_top=True)
